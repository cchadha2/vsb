...
def main():
    ...
    print('Signal data shape: {}'.format(train_sig.shape))
    ...
    df = train_sampling(
        train_sig,
        train_meta,
        '../output/train_sig_sampled.parquet',
        '../output/train_sampled.csv')

    fc = {'symmetry_looking': [{'r': 1}],
          'abs_energy': None,
          'mean': None,
          'number_cwt_peaks': [{'n': 100}],
          'number_peaks': [{'n': 100}],
    ...
    del array_denoised
    df.to_parquet('../output/train_sig_2500hz_4th_order_dwt.parquet')
    print('De-noised data')

    df = tsfresh_formatting(df, train_meta)
    ...
    pd.concat([tsfresh_df, train_meta], axis=1).to_csv(
        '../data/processed_train_2500Hz_' + version + '.csv', index=False)
