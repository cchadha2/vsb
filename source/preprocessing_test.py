...
def main():
    ...
    for column in range(test_column_start + stepsize *
        ...
        df = suppress_sine(test_sig_chunk)
        del test_sig_chunk
        # df.to_parquet('../output/test_sig_2500hz_4th_order_' + str(test_chunk) + '.parquet')
        print('Suppressed sine')
        print('DataFrame shape: {}'.format(df.shape))

        # Seems to add duplicate column at end of array
        array_denoised = denoise_signal(df, wavelet='haar', level=1)
        if test_chunk == chunks:
            array_denoised = array_denoised[:, :-1]
        print('Denoised array shape: {}'.format(array_denoised.shape))
        df = pd.DataFrame(array_denoised, columns=df.columns.tolist())
        del array_denoised
        df.to_parquet('../output/test_sig_2500hz_4th_order_dwt_' +
                      str(test_chunk) + '.parquet')
        print('De-noised data')
        print('DataFrame shape: {}'.format(df.shape))
        ...
        pd.merge(
            ...
            test_meta,
            on='signal_id').to_csv(
                '../output/2500Hz_1.1/processed_test_2500Hz_chunk_' +
                str(test_chunk) +
                '.csv',
            index=False)
    ...
    test_chunks = glob.glob(
        "../output/2500Hz_1.1/processed_test_2500Hz_chunk_*.csv")
