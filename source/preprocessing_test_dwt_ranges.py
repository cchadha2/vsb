...
def tsfresh_formatting(df_sig, df_meta):
    ...
    amplitude = pd.DataFrame(df_sig.stack().reset_index().drop('level_0', axis=1).rename(columns={'level_1': 'signal_id'}))
    amplitude['signal_id'] = amplitude['signal_id'].astype('int')

    signal_id = pd.DataFrame(df_meta['signal_id']).reset_index(drop=True)
    tsfresh_df = pd.merge(signal_id, amplitude, how='inner').rename(columns={0: 'amplitude'})
...
def main():
    ...
    for column in range(test_column_start + stepsize *(test_chunk - 1), test_column_end + 1, stepsize):
        print('Starting chunk processing {}'.format(test_chunk))
        signals = [str(cols) for cols in range(column, column + stepsize) if cols <= test_column_end]
        ...
        #
        # print('Test chunk: {}'.format(test_chunk))
        ...
        for rows in range(0,800000,stepsize):
            ...
            df_range = tsfresh_formatting(df_range, test_meta)
            ...
            df_range = pd.merge(tsfresh_df.reset_index().rename(columns={'id': 'signal_id'}), test, on='signal_id')
        ...
        df_chunk = df_chunk.rename(columns={'signal_id_1': 'signal_id'})\
                           .drop(['signal_id_2', 'signal_id_3', 'signal_id_4'],
                                 axis=1)

        processed_test = pd.concat([processed_test, df_chunk])
        print(processed_test.head())

        test_chunk += 1

        processed_test.to_csv('../data/processed_test_2500Hz_' + version + '.csv', index_label=False)
