...
train_sig_filtered = pd.DataFrame(data=filtered_sig, columns = train_sig.columns.tolist())
...
for i in signals_to_plot:
    plt.figure()
    sns.lineplot(train_sig_filtered[:800000].index, train_sig_filtered[str(i)][:800000])
    plt.show()
